...
function kakaochat(record: string[]) {
    const answer: string[] = [];
    const map: { [key: string]: string } = {};
    record.forEach(e => {
        const[action, uid, name] = e.split(" ");
    ...
    record.forEach(e => {
        const[action, uid, name] = e.split(" ");
        action ==="Enter" ? answer.push(`${map[uid]}님이 들어왔습니다.`) : action === "Leave" ? answer.push(`${map[uid]}님이 나갔습니다.`) : null ;
    });
